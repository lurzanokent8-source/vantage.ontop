    ...
    <div class="viewport">
        ...
        <div id="exec" class="tab hidden">
            <div class="card" style="max-width: 700px; margin: auto;">
                ...
                <textarea id="code_area" placeholder="-- REQUIRE HERE"></textarea>
                <button class="btn" onclick="execute()">RUN SERVER-SIDE</button>
            </div>
        </div>

        <div id="white" class="tab hidden">
            <div id="auth_ui" class="card" style="max-width: 400px; margin: auto; text-align: center;">
                <input id="w_input" placeholder="Roblox Username">
                <button class="btn" onclick="addWhite()">AUTHORIZE PLAYER</button>
                <div id="profile_box" class="hidden" style="margin-top:20px;">
                    <img id="p_img" src="" style="width:100px; border-radius:50%; border:2px solid var(--gold);">
                    <h3 id="p_name" style="color:var(--gold);"></h3>
        ...
        <div id="settings" class="tab hidden">
            <div class="card" style="max-width: 450px; margin: auto;">
                <input id="bg_input" placeholder="Background URL">
                <button class="btn" onclick="setBg()">SAVE THEME</button>
                <button class="btn" style="background:#111; color:#fff;" onclick="toggleAes()">TOGGLE AESTHETIC</button>
                <button class="btn" style="background:#ff4444; color:#fff;" onclick="location.href='/logout'">LOGOUT</button>
            </div>
        </div>
...
<script>
    ...
    function genKey() {
        fetch('/api/admin/gen_key', {method:'POST'}).then(r => r.json()).then(data => {
            document.getElementById('key_list').innerHTML += `<span class="key-badge">${data.key}</span>`;
        });
    }

    function setBg() {
        const url = document.getElementById('bg_input').value;
        document.getElementById('main_body').style.backgroundImage = `url('${url}')`;
        fetch('/api/set_setting', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({bg: url})});
    }
    ...
    function execute() { fetch('/api/execute', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({code: document.getElementById('code_area').value})}).then(()=>alert("Sent")); }
...
def index():
    ...
    if sid and sid not in user_sessions:
        user_sessions[sid] = {"whitelist": None, "queue": [], "aesthetic": False, "bg": ""}
...
def roblox_sync():
    ...
    for sid, info in user_sessions.items():
        if info["whitelist"] and info["whitelist"]["name"] in players:
