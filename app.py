...
.searching, .executing { animation: pulse 0.8s infinite; background: #333 !important; color: #fff !important; pointer-events: none; }
...
<script>
    ...
    // --- THE FIX FOR DIRECT JOINING (PC AND MOBILE) ---
    function launchRoblox(jid, pid) {
        const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
        if (isMobile) {
            // Mobile Deeplink logic
            window.location.href = `roblox://placeId=${pid}&gameInstanceId=${jid}`;
        } else {
            // PC Launcher logic
    ...
    function togglePrivacy() { 
        privacyOn = !privacyOn; 
        sync(); // Refresh immediately to apply/remove blur
    }
        ...
        fetch('/api/global_signals').then(r => r.json()).then(data => {
            ...
            if (Object.keys(data.servers).length === 0) {
                ...
                for(let id in data.servers) { 
                    ...
                    // Call the NEW launchRoblox function instead of window.location
